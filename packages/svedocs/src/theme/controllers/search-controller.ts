...
import { filterSearchRecords } from '../../search/scope.js';
import type { LocalSearchAdapter } from '../../search/client-types.js';
...
export function createSearchControllerWithAdapter(initial: SvedocsSearchControllerOptions, local: LocalSearchAdapter): SvedocsSearchController {
  ...
  const localResults = derived(
    [query, loadedRecords, options, remoteStatus, open],
    ([$query, $records, $options, $remoteStatus, $open], set) => {
      let active = true;
      if (isRemoteSearch($options) && $query.trim() && $remoteStatus !== 'error') {
        set([]);
        return;
      }
      if (local.prepare && !$open) { set([]); return; }
      if (!$query.trim()) {
        set(createDefaultSearchResults($records, $options.scope).map(sanitizeSearchResult));
        if ($open && !isRemoteSearch($options) && $records.length && local.prepare) {
          recordsStatus.set('loading');
          void local.prepare($records).then(() => { if (active) recordsStatus.set('ready'); })
            .catch(() => { if (active) recordsStatus.set('error'); });
        }
        return () => { active = false; };
      }
      const next = local.search($records, { query: $query, limit: 8, ...$options.scope });
      if (next instanceof Promise) {
        recordsStatus.set('loading');
        set([]);
        void next.then((hits) => { if (active) { recordsStatus.set('ready'); set(hits.map(sanitizeSearchResult)); } })
          .catch(() => { if (active) { recordsStatus.set('error'); set([]); } });
      } else set(next.map(sanitizeSearchResult));
      return () => { active = false; };
    },
    [] as SearchResult[]
  );
  ...
  const unsubscribe = results.subscribe((nextResults) => {
  ...
  function setOptions(nextOptions: Partial<SvedocsSearchControllerOptions>): void {
    ...
    if (recordsChanged || loadRecordsChanged) local.release?.();
  ...
  function show(): void {
    open.set(true);
    if (!isRemoteSearch(get(options))) void ensureRecords();
    syncRemoteResults();
  }
