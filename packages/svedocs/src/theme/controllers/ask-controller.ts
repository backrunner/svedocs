...
export function createAskAiControllerWithAdapter(initial: SvedocsAskAiControllerOptions, local: LocalSearchAdapter): SvedocsAskAiController {
  ...
  function setOptions(nextOptions: Partial<SvedocsAskAiControllerOptions>): void {
    ...
    if (recordsChanged || loadRecordsChanged) { cancelAskRequest(); local.release?.(); }
  ...
  async function send(text = get(input)): Promise<void> {
    ...
    if (currentOptions.buildMode !== 'edge') {
      ...
      const draft = await createLocalAskDraft(value, sourceRecords, currentOptions.scope, currentOptions.t);
      if (requestVersion !== conversationVersion) return;
      updateAssistant(assistantMsg.id, draft);
      loading.set(false);
      return;
    }
    ...
    try {
      ...
      const sourceRecords = await ensureRecords();
      if (requestVersion !== conversationVersion || requestController.signal.aborted) return;
      const ranked = await rankRecords(sourceRecords, value.toLowerCase(), currentOptions.scope);
      if (requestVersion !== conversationVersion) return;
      const fallbackCitations = ranked.slice(0, 3).map((result) => ({
        title: result.title,
  ...
  async function createLocalAskDraft(question: string, records: SvedocsSearchRecord[], scope: SearchScope, t: SvedocsTranslate): Promise<Partial<SvedocsAskAiMessage>> {
    const citations = (await rankRecords(records, question.toLowerCase(), scope)).slice(0, 3).map((result) => ({
      title: result.title,
      url: sanitizeNavigationUrl(result.url),
      ...(result.section ? { section: result.section } : {})
    }));
  ...
  async function rankRecords(records: SvedocsSearchRecord[], query: string, scope: SearchScope): Promise<SearchResult[]> {
    return Promise.resolve(local.search(records, { query, limit: 5, ...scope })).catch(() => []);
  }
