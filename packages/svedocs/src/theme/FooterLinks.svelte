<script lang="ts">
  ...
  $: footer = context.config.theme.footer;

  function isGithubFooterLink(item: { label: string; href: string }): boolean {
    return item.href.toLowerCase().includes('github.com')
      || item.label.trim().toLowerCase().includes('github');
  }
</script>
...
{#if footer !== false && footer.links.length > 0}
  <nav aria-label={context.t('nav.footer')} data-theme-component="footer-links">
    {#each footer.links as item}
      ...
      {#if isGithubFooterLink(localized)}
        <a
      ...
      {:else}
        <a href={localized.href} rel={linkRel(localized)} target={localized.external ? '_blank' : undefined}>{localized.label}</a>
      {/if}
    {/each}
  </nav>
{/if}
