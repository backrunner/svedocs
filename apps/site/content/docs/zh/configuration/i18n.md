...
对于品牌、顶部导航、社交链接、页脚和首页操作中的站内链接，默认主题会查找当前语言下的对应页面。因此中文页面上的 `/docs/guides` 配置链接，在译文存在时会变成 `/docs/zh/guides`。

语言切换器使用相同的页面映射。如果当前页面没有某种语言的译文，对应选项会被禁用，不会把读者送到无关页面。其他配置链接找不到译文时，则继续使用原始 URL。

Markdown 链接属于作者内容，不会被框架隐式重写。镜像文件之间建议使用相对链接：

```md
继续阅读[部署指南](../guides/deploy.md)。
```

绝对链接 `/docs/guides/deploy` 会明确指向这个路由，它可能属于默认语言。
